...
_filter_u_rm_Advance() { # filter file or dir
    ...
    if [ -d $_trash_dir/$1 ]; then
        ...
        local _path_fileT="$(dirname $(command tail -n 1 $_trash_file/$1))"
...
_option_u_rm_Advance() { # option -u --undo
    if [ ${#options_rm[@]} -eq 1 ]; then
        if [ $options_rm[@] = "--undo" -o $options_rm[@] = "-u" ]; then
            ...
            echo "$_error_rm_ [option: ${options_rm[@]}] :bad substitution." && return 1
        fi
    else
        echo "$_error_rm_ [option: ${options_rm[@]}] :bad substitution." && return 1
    fi
