_content_my_cache() {
    ...
    if [ -f $1 ]; then
        ...
        echo $(dirname $(realpath $1)) >>$1/.my_cache
...
_delete_rm_Advance() {
    ...
    if [ -d $1 ]; then # delete -> directory
        ...
        [ -f $_trash_file/"$(basename $1)" ] && {
            ...
            local _End_line_file_T=$(wc $_trash_file/"$(basename $1)" | awk '{print $1}')

            mv $_trash_file/"$(basename $1)" $_trash_file/"$(basename $1)".$_new_name_T &&          # rename
                sed -i.temp -e ''$_End_line_file_T'd' $_trash_file/"$(basename $1)".$_new_name_T && # remove old path
...
_main_rm_Advance() {
    if [ ! -d $1 -a ! -f $1 ]; then
        ...
        [ $? -eq 0 ] && echo "  [option: ${options_rm[@]}] :bad substitution."
        return 1
    else
    ...
    if [ $(command ls -ld $1 | awk '{print $3}') = $(whoami) ]; then
        ...
        printf "-> $(dirname $(realpath $1))/\033[0;31m$1($(command ls -ld $1 | awk '{print $3}'))\033[0m\033[3;32m is not yours, to skip[y/n]:\033[0m"
        read -q tf
        echo
        if [ $tf = "y" ]; then
            [ -d $1 ] && {
